...
void IntersectionUI::writeTest() {
    ...
    for (int i = seed; i < (seed + 1638); i++) {
		...
		cubeHr = shape->intersect(p, dir);
		cubeHr.sortHits();
		shape = make_shared<Cylinder>(tess1, tess2);
		cylinderHr = shape->intersect(p, dir);
		cylinderHr.sortHits();
		shape = make_shared<Cone>(tess1, tess2);
		coneHr = shape->intersect(p, dir);
		coneHr.sortHits();
		shape = make_shared<Sphere>(tess1);
		sphereHr = shape->intersect(p, dir);
		sphereHr.sortHits();
