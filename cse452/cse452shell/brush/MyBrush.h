...
private:
	...
	int scanLine(int y, double s, ScreenPoint point);
	int maxPointY(vector<ScreenPoint> a);
	int maxPointX(vector<ScreenPoint> a);
	int minPointY(vector<ScreenPoint> a);
	int minPointX(vector<ScreenPoint> a);
	void horizontal(int, Color);
	void vertical(int, Color);
	int isvertex(int, vector<ScreenPoint>, int);
	void scanRegion(int maxY, int minY, vector<ScreenPoint>points, Color col);
};
